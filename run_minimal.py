...
OFFICIAL = {
    ...
    #autoencoder architecture for drebin
    "ae_hidden_dim": 512,
    "ae_encoding_dim": 32,
...
DREBIN_FAMILY_CONCEPTS = {
    "FakeInstaller": [0, 1, 7, 8, 9],
    "DroidKungFu": [0, 2, 6],
    "Plankton": [0, 2, 7, 8],
    "GingerMaster": [0, 6],
    "BaseBridge": [0, 1, 6, 9],
...
def build_drebin_concepts_from_family_names(y_raw, family_names): 
#y_raw: original family id array, shape [N]
#family_names: array/list where family_names[id] gives the family name
#returns: concepts: [N, 10], 0/1  ;  concept_mask: [N, 10], 1 is valid label
    
    concepts = np.zeros((len(y_raw), len(CONCEPT_NAMES)), dtype=np.float32)
    concept_mask = np.ones_like(concepts, dtype=np.float32)
    for i, fam_id in enumerate(y_raw):
        fam_name = family_names[int(fam_id)]
        if isinstance(fam_name, bytes):
            fam_name = fam_name.decode("utf-8")
        fam_name = str(fam_name)
        if fam_name not in DREBIN_FAMILY_CONCEPTS:
            raise ValueError(
                f"Unknown Drebin family name: {fam_name}. "
                f"family_names must match Table 2 names: {list(DREBIN_FAMILY_CONCEPTS.keys())}"
            )
        for b in DREBIN_FAMILY_CONCEPTS[fam_name]:
            concepts[i, b] = 1.0
    return concepts, concept_mask

##read data and do hold out split

DATA_PATH = "data/drebin_features.npz"

if not os.path.exists(DATA_PATH):
    raise FileNotFoundError(
        f"Cannot find {DATA_PATH}. "
        "missing .npz file with X, y, family_names"
    )
data = np.load(DATA_PATH, allow_pickle=True)
X = data["X"].astype(np.float32)
y = data["y"].astype(np.int64)

if "family_names" not in data:
    raise KeyError("drebin_features.npz must contain family_names.")

family_names = data["family_names"]
concepts, concept_mask = build_drebin_concepts_from_family_names(y, family_names)
print("X:", X.shape)
print("y:", y.shape)
...
def expand_drebin_classifier(old_model, new_num_classes):
...
def dream_joint_adaptation_with_schedule(
    ...
    classifier = expand_drebin_classifier(old_classifier, num_total_classes).to(device)
...
torch.save(
    ...
    "checkpoints/dream_drebin_official_like_pytorch.pt",
)
np.savez(
